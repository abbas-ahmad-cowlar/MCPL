...
\For{$iter = 1$ to $\text{MAX\_ITER}$}
    ...
    \EndIf
    \State
    \State \textbf{6. INTENSIFICATION (every INTENSIFY\_FREQ iterations):}
    \If{$iter \mod \text{INTENSIFY\_FREQ} = 0$}
        \State Run local search from current $S$ for 10 iterations \Comment{Exploit current region thoroughly}
        \State Update $S^*$ if local search finds improvement
    \EndIf
    \State
    \State \textbf{7. DIVERSIFICATION (when stagnation $\geq$ STAGNATION\_LIMIT):}
