...
\begin{itemize}
  ...
  \item \textbf{Key Parameters}: \texttt{MAX\_MOVES} (default 200), \texttt{INIT\_METHOD} (default "greedy").
\end{itemize}

\subsubsection{Multi-Start Local Search: \texttt{mclp\_multistart.mos}}
\begin{itemize}
  \item \textbf{Description}: Runs local search from multiple diverse initial solutions.
  \item \textbf{Usage}: \texttt{mosel exec mclp\_multistart.mos "DATA\_FILE=data/L1.dat"}
  \item \textbf{Key Parameters}: \texttt{N\_STARTS} (default 10), \texttt{MAX\_MOVES} (default 200), \texttt{BASE\_SEED} (default 42).
\end{itemize}
